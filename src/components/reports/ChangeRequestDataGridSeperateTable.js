...
const headCellsCalibration = [
  ...
  },
  {
    ...
    renderCell: (param) => {
      const currentRow = param.row;
      return currentRow?.calibration?.policyParameters?.length > 0 ? <CalibrationPolicyParamDialog items={currentRow?.calibration?.policyParameters} title="Policy Parameters" /> : ""
    },
  },
  {
    field: 'manage_voilations',
...
export default function ScannedReportsDataGrid() {
  ...
  headCellsCalibration[headCellsCalibration.length-3].renderCell = (param) => {
    const currentRow = param.row;
    let vid = []; 
    currentRow.violations.forEach(element => {
      vid.push(element.id); 
  ...
  let remediationRows=  rows?.map( object => object?.type === "remediation"); 
