...
const headCells = [
  ...
  },
  {
    ...
    renderCell: (param) => {
      ...
      <a target={'_blank'} href={currentRow.link}> {currentRow.ID}</a>
    </Tooltip>
    }
  },
...
export default function ScannedReportsDataGrid() {
  ...
  React.useEffect(() => {
    ...
    UserService.getCves().then((results) => {
      let resultsTemp = []; 
      if (results.status === 200) {
        resultsTemp = results.data; 
        resultsTemp.forEach(element => {
          element.id = element.ID; 
          element.link = `https://nvd.nist.gov/vuln/detail/${element.ID}`; 
          
        });
        setCveRows(resultsTemp);
